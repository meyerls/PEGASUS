...
class PegasusSetup(object):
    ...
    def create_camera_trajectory(self, num_cameras=5, num_interpolation_steps=24, random=False):
        cams = []

        start_frame = np.random.randint(0, self.cam_extr.keys().__len__() - num_cameras)

        for pose_idx in range(start_frame, start_frame + num_cameras + 1):

            idx = sorted(self.cam_extr.keys())[pose_idx]
            idx_next = sorted(self.cam_extr.keys())[pose_idx]
...
class PegasusBOPDatasetWriter:
    def __init__(self, dataset_name: str,
        ...
        self.sem_mask_path = self.scene_path / 'sem_mask'
        self.sem_mask_path.mkdir(parents=True, exist_ok=True)
    ...
    def write_training_data(self, rgb_image: np.ndarray,
        ...
        if isinstance(semantic_masks, np.ndarray):
            seg_file_name = self.sem_mask_path / "{:06d}.png".format(frame_id)
            imageio.imwrite(seg_file_name, semantic_masks)
    ...
    def add_scene_gt_json(self, time_step, gs_object_list, cam, rgb_image, debug=False):
        ...
        for gs_object_id in gs_object_list.keys():
            mesh = o3d.io.read_triangle_mesh(gs_object_list[gs_object_id].meta_info.urdf_obj_path)
            ...
            T_m2w = np.eye(4)
            T_m2w[:3, :3] = gs_object_list[gs_object_id].R_init.cpu().numpy()
            T_m2w[:3, -1] = gs_object_list[gs_object_id].t_init.cpu().numpy()  # - mesh.get_center()
            ...
            self.scene_gt_json[str(time_step)].append({
                ...
                'obj_id': gs_object_list[gs_object_id].meta_info.ID,
                'bullet_obj_id': gs_object_id,
...
def gaussian_splatting_manipulation(physics_file: str,
    ...
    pegasus_dataset = PegasusBOPDatasetWriter(dataset_name=dataset_name,
                                              ...
                                              scene_id=scene_id)
    viewport_cam_list = pegasus_setup.create_camera_trajectory(num_cameras=num_cameras,
    ...
    pegasus_setup.init_video_streams(output=pegasus_dataset.dataset_path / 'video/{:06d}'.format(scene_id), fps=20)
    ...
    with (torch.no_grad()):
        ...
        for i in range(len(viewport_cam_list)):
            ...
            if i < len(viewport_cam_list):
                ...
                if save_bop:
                    pegasus_dataset.write_training_data(
                        ...
                        semantic_masks=(semantic_segmentation_mask.permute((1, 2, 0)) * 255).numpy().astype('uint8'),
...
def write_models(dataset_path: str, object_list, model_path: str):
    ...
    for obj in object_list:
        obj_id = obj.ID
        model_id_list.append(obj_id)

        object_name = obj.OBJECT_NAME
        object_mesh_path = Path(dataset_path) / 'urdf' / (object_name + '.obj')
        obj_mesh = o3d.io.read_triangle_mesh(object_mesh_path.__str__())
        axis_aligned_bbox = obj_mesh.get_axis_aligned_bounding_box()

        model_info_json.update(
            {str(obj_id): {"diameter": calculate_diameter_of_object(obj=obj_mesh),
                           "min_x": axis_aligned_bbox.min_bound[0],
                           "min_y": axis_aligned_bbox.min_bound[1],
                           "min_z": axis_aligned_bbox.min_bound[2],
                           "size_x": axis_aligned_bbox.get_extent()[0],
        ...
        ply_output_path = model_path / 'obj_{:06d}.ply'.format(obj_id)
        o3d.io.write_triangle_mesh(ply_output_path.__str__(), obj_mesh, write_ascii=True)

    print("Using models: {}".format(model_id_list))
    with open(Path(model_path / 'models_info.json'), 'w') as f:
        json.dump(model_info_json, f, indent=1)
...
if __name__ == '__main__':
    ...
    env1 = MannholeCover(dataset_path=DATASET_PATH)
    env2 = Cobblestone(dataset_path=DATASET_PATH)
    env3 = PlainTableSetup(dataset_path=DATASET_PATH)
    env4 = Asphalt(dataset_path=DATASET_PATH)
    env5 = Tiles(dataset_path=DATASET_PATH)
    # env6 = Grass(dataset_path=DATASET_PATH)
    ...
    if False:
        ...
        obj_list = [obj1, obj2, obj4, obj5, obj6, obj8, obj11, obj12, obj13, obj14, obj15]

    if False:
        obj1 = CupNoodle02(dataset_path=DATASET_PATH)
        obj2 = CupNoodle03(dataset_path=DATASET_PATH)
        obj3 = CupNoodle04(dataset_path=DATASET_PATH)
        obj4 = CupNoodle05(dataset_path=DATASET_PATH)
        obj5 = CupNoodle06(dataset_path=DATASET_PATH)

        env_list = [env3, env3, env3, env3]
        obj_list = [obj4]
    ...
    if True:
        ...
        obj115 = CupNoodle16(dataset_path=DATASET_PATH)
        obj115 = CupNoodle17(dataset_path=DATASET_PATH)
        obj115 = CupNoodle18(dataset_path=DATASET_PATH)
        obj115 = CupNoodle19(dataset_path=DATASET_PATH)
        obj115 = CupNoodle20(dataset_path=DATASET_PATH)
        #obj_list = [
        #    obj101, obj102, obj103,
        #    obj104, obj105, obj106,
        #    obj107, obj108, obj109,
        #    obj110, obj111, obj112,
        #    obj113, obj114, obj115,
        #    obj116, obj117, obj118,
        #    obj119, obj120
        #]
        obj_list = [
            obj1
    ...
    dataset_name = 'pegasus_ycb'
    write_models(dataset_path=DATASET_PATH, object_list=obj_list, model_path="./dataset/{}/models".format(dataset_name))
    ...
    for scene_id in range(1, 300):
        py_engine = PybulletEngine(asset_folder=URDF_ASSET_FOLDER,
                                   output_path_json='./dataset/{}/engine/{:06d}_simulation_steps.json'.format(
        ...
        # max_objects = np.min([obj_list.__len__(), 8])
        # max_objects = np.min([obj_list.__len__(), 12])
        # num_objects = np.random.randint(2, max_objects)
        # num_objects = np.random.randint(6, max_objects)
        # random_objects_idx = np.unique(np.random.randint(0, obj_list.__len__(), num_objects)).tolist()

        num_objects = np.random.randint(1, 2)
        # random_objects_idx = np.unique(np.random.randint(0, 1, num_objects)).tolist()
        random_objects_idx = np.unique(np.random.randint(0, obj_list.__len__(), num_objects)).tolist()
        ...
        py_engine.add_object(object_instance=select_env, start_pos=select_env.START_POSITION_PYBULLET)
        for obj in random_objects:
            py_engine.add_object(object_instance=obj, start_pos=select_env.define_start_pos())

        py_engine.simulate()

        gaussian_splatting_manipulation(physics_file=py_engine.trajectory_path,
                                        dataset_path=DATASET_PATH,
